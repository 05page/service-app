...
export function ComissionSection() {
    const [searchTerm, setSearchTerm] = useState("")
    const filtreCommision = commissionsData.filter(commission => commission.nom.toLowerCase().includes(searchTerm.toLowerCase())
    );
    const [activeTab, setActiveTab] = useState("overview");

    const totalCommissionDues = commissionsData.reduce((acc, c) => acc + c.commissionDue, 0);
    const totalCommissionPayees = commissionsData.reduce((acc, c) => acc + c.commissionPayee, 0);
    const totalCommissionAttentes = commissionsData.filter(c => c.statut === "en attente").length;
        ...
        <div className="space-y-6">
            ...
            <div className="grid gap-4 md:grid-cols-4">
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{(totalCommissionDues - totalCommissionPayees).toLocaleString()} €</div>
                        <p className="text-xs text-muted-foreground">
                            Sur {totalCommissionDues.toLocaleString()} Fcfa dus
                        </p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{totalCommissionPayees.toLocaleString()} Fcfa</div>
                        <p className="text-xs text-muted-foreground">
                            {((totalCommissionPayees / totalCommissionDues) * 100).toFixed(1)}% du total
                        </p>
                    </CardContent>
                </Card>
                ...
                <Card>
                    ...
                    <CardContent>
                        <div className="text-2xl font-bold">{totalCommissionAttentes}</div>
                        <p className="text-xs text-muted-foreground">
            ...
            <Tabs value={activeTab} onValueChange={setActiveTab}>
                ...
                <TabsContent value="overview" className="space-y-4">
                    ...
                    <div className="space-y-4">
                        {filtreCommision.map((commission) => (
